...
public class OrderEditVmd {
	...
	@Init
	@NotifyChange(value = { "trOrderHeaderDto", "mstEmployeeDto",
			"listCustomers", "mstCustomerDto", "listOrderDetails",
			"listProducts", "visible", "readonly" })
	public void load() {
		trOrderHeaderDto = (TrOrderHeaderDto) Sessions.getCurrent()
				.getAttribute("objOrderHeader");
		mstEmployeeDto = (MstEmployeeDto) Sessions.getCurrent().getAttribute(
				"user");

		if (trOrderHeaderDto != null && trOrderHeaderDto.getNoNota() != null
				&& !trOrderHeaderDto.getNoNota().equalsIgnoreCase("")) {
			try {
				mstCustomerDto = mstCustomerSvc
						.findByCustomerCode(trOrderHeaderDto.getCustomer()
								.getCustomerCode());
				...
				listOrderDetails = trOrderDetailSvc
						.findByNoNota(trOrderHeaderDto.getNoNota());
			...
			try {
				...
				if (trOrderHeaderDto.getNoNota() != null
						&& !trOrderHeaderDto.getNoNota().equalsIgnoreCase("")) {
					// setVisible(Boolean.FALSE);
					setReadonly(Boolean.TRUE);
				}
				...
				if (trOrderDetailSvc.findByNoNota(trOrderHeaderDto.getNoNota()) != null
						&& !trOrderDetailSvc.findByNoNota(
								trOrderHeaderDto.getNoNota()).isEmpty()
						&& trOrderDetailSvc.findByNoNota(
								trOrderHeaderDto.getNoNota()).size() > 0) {
					listOrderDetails = trOrderDetailSvc
							.findByNoNota(trOrderHeaderDto.getNoNota());
				} else {
	...
	@Command(value = "save")
	public void save() {
		TrOrderHeaderDto findHeader = trOrderHeaderSvc
				.findByNoNota(trOrderHeaderDto.getNoNota());
		...
		if (findHeader == null && findHeader.getNoNota().equalsIgnoreCase("")) {
			try {
				...
				for (TrOrderDetailDto detail : listOrderDetails) {
					...
					mstProductDto = mstProductSvc
							.findByProductCode(productCode);

					Long stokAkhir = mstProductDto.getQuantity()
							- detail.getQty();

					if (stokAkhir < 0) {
						Messagebox
								.show("Pembelian tidak dapat melebihi jumlah stok.");
					} else {
						mstProductDto.setQuantity(stokAkhir);
						mstProductDto.setProductCode(productCode);
						trOrderDetailSvc.save(detail);
						TrOrderDetailDto detailDto = trOrderDetailSvc
								.findOneById(detail.getDetailId());
						if (detailDto != null) {
							mstProductSvc.update(mstProductDto);
						}
						Clients.showNotification("Data berhasil disimpan.",
								Clients.NOTIFICATION_TYPE_INFO, null, null,
								1500);
		...
		} else if (findHeader != null
				&& !findHeader.getNoNota().equalsIgnoreCase("")) {
	...
	@NotifyChange(value = { "listOrderDetails", "statusPopup",
			"trOrderHeaderDto", "tampungHargaTotal" })
	...
	public void hitungSubTotal() {
		if (trOrderDetailDto.getUnitPrice().doubleValue() != 0
				&& trOrderDetailDto.getQty().intValue() != 0
				&& trOrderDetailDto.getDiscount().intValue() == 0) {
			Double tampungSub = trOrderDetailDto
					.getUnitPrice()
					.multiply(
							new BigDecimal(trOrderDetailDto.getQty().intValue()))
					.doubleValue();
			tampungSubTotal = new BigDecimal(tampungSub);
			// tampungSubTotal = trOrderDetailDto.getUnitPrice().multiply(new
			// BigDecimal(trOrderDetailDto.getQty().intValue()));
			// tampungSubTotal = trOrderDetailDto.getUnitPrice().multiply(new
			// BigDecimal(trOrderDetailDto.getQty()));
			System.out.println("SubTotal without discount");
			trOrderDetailDto.setSubtotal(tampungSubTotal);
		} else if (trOrderDetailDto.getUnitPrice().doubleValue() != 0
				&& trOrderDetailDto.getQty().intValue() != 0
				&& trOrderDetailDto.getDiscount().intValue() != 0) {
			...
			// BigDecimal(trOrderDetailDto.getQty()));
			System.out
					.println("Unit Price: " + trOrderDetailDto.getUnitPrice());
			System.out.println("Qty: " + trOrderDetailDto.getQty());
			Double discount = Double.valueOf(trOrderDetailDto.getDiscount()
					.intValue() / 100.0);
			System.out.println("Discount Perc Sub: " + discount);
			Double tampungDisc = trOrderDetailDto
					.getUnitPrice()
					.multiply(
							new BigDecimal(trOrderDetailDto.getQty().intValue()))
					.multiply(
							new BigDecimal((trOrderDetailDto.getDiscount()
									.intValue() / 100.0))).doubleValue();
			System.out.println("Discount subtotal: " + tampungDisc);
			// BigDecimal tampungDisc =
			// trOrderDetailDto.getUnitPrice().multiply(new
			// BigDecimal(trOrderDetailDto.getQty())).multiply(new
			// BigDecimal(trOrderDetailDto.getDiscount()).divide(new
			// BigDecimal(100)));
	...
	public void hitungHargaTotal() {
		...
		if (!trOrderHeaderDto.getGlobalDiscount().equals(0)) {
			// BigDecimal tampungDisc =
			// trOrderHeaderDto.getTotalPrice().multiply(new
			// BigDecimal(trOrderHeaderDto.getGlobalDiscount()).divide(new
			// BigDecimal(100)));
			Double discount = (double) (trOrderHeaderDto.getGlobalDiscount() / 100.0);
			System.out.println("Discount Percentage: " + discount);
			Double tampungDisc = trOrderHeaderDto.getTotalPrice()
					.multiply(new BigDecimal(discount)).doubleValue();
			System.out.println("Discount: " + tampungDisc);
			tampungSub = trOrderHeaderDto.getTotalPrice().doubleValue()
					- tampungDisc;
	...
	public void tampilStock() {
		if (trOrderDetailDto.getProduct() != null
				&& trOrderDetailDto.getProduct().getProductCode() != null
				&& !trOrderDetailDto.getProduct().getProductCode()
						.equalsIgnoreCase("")) {
			mstProductDto = new MstProductDto();
			mstProductDto = mstProductSvc.findByProductCode(trOrderDetailDto
